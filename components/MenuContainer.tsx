...
const MenuContainer = ({ title, imageSrc, setType, type }: MenuContainerProps, { }: TouchableOpacityProps) => {
  ...
  return (
    <TouchableOpacity className='items-center justify-center space-y-1' onPress={handlePress}>
      <View className={`w-20 h-20 rounded-full items-center justify-center ${ type === title.toLowerCase()? 'w-23 h-23' : ""}`}>
        <Image source={imageSrc} className={`w-full h-full object-contain  ${type === title.toLowerCase()?'opacity-100':'opacity-80'}`} />
      </View>
